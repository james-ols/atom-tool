    ...
    private function buildRouter(Session $session): Router
    {
        ...
        $router->get('/', $requireAuth(function (Request $request, Session $session): Response {
            ...
            return Response::html($this->render('dashboard', [
                ...
                'uploads'       => $uploads,
                'mappingPath'   => $this->config->mappingPath(),
            ]));
        }));
        ...
        // Preflight report download. Serves the stored <runId>.preflight.txt for
        // audit purposes and provenance (the JSON coverage report generated by the
        // /run handler). Guarded exactly like /download so a crafted runId can't
        // escape the reports area.
        $router->get('/report', $requireAuth(function (Request $request, Session $session): Response {
            $runId = (string) $request->queryParam('runId', '');
            if ($runId === '') {
                return Response::notFound('Missing run identifier.');
            }
            ...
            return (new Response())
                ...
                ->header('Content-Length', (string) strlen($report))
                ->body($report);
        }));
